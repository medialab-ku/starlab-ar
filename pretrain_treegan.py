        ...
        for epoch in range(epoch_log, self.args.epochs):
            ...
            if self.args.eval_every_n_epoch > 0:
                if epoch % self.args.eval_every_n_epoch == 0 :
                    checkpoint_eval(self.G, self.args.device, n_samples=self.args.n_samples, batch_size=self.args.batch_size,conditional=False, ratio='even', FPD_path=self.args.FPD_path,class_choices=self.args.class_choice)
...
if __name__ == '__main__':
    ...
    SAVE_CHECKPOINT = os.path.join(args.ckpt_path, args.ckpt_save)
